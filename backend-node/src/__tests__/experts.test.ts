...
import { ConfigurableExpert, createExpert, listExperts } from '../experts';
...
describe('专家系统', () => {
  ...
  describe('资深作者', () => {
    it('应该生成发言', async () => {
      const def = expertLoader.getExpert('senior_author_v1')!;
      const expert = new ConfigurableExpert(def, ExpertRole.MAIN, Granularity.CHAPTER);
  ...
  describe('读者代表', () => {
    it('应该生成读者视角的发言', async () => {
      const def = expertLoader.getExpert('reader_representative_v1')!;
      const expert = new ConfigurableExpert(def, ExpertRole.REVIEW, Granularity.CHAPTER);
      const opinion = await expert.speak({
        vision: { coreIdea: '测试故事' }
      });
  ...
  describe('剧情架构师', () => {
    it('应该分析故事结构', async () => {
      const def = expertLoader.getExpert('plot_architect_v1')!;
      const expert = new ConfigurableExpert(def, ExpertRole.MAIN, Granularity.CHAPTER);
      const opinion = await expert.speak({
        vision: { coreIdea: '测试故事' }
      });
  ...
  describe('不同粒度', () => {
    it('卷级粒度应该有不同的提示', async () => {
      const def = expertLoader.getExpert('senior_author_v1')!;
      const expert = new ConfigurableExpert(def, ExpertRole.MAIN, Granularity.VOLUME);
      const opinion = await expert.speak({});
    ...
    it('场景级粒度应该有不同的提示', async () => {
      const def = expertLoader.getExpert('senior_author_v1')!;
      const expert = new ConfigurableExpert(def, ExpertRole.MAIN, Granularity.SCENE);
      const opinion = await expert.speak({});

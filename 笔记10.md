...
1. **前端技术栈**：React/Vue？是否参考 Dify 的 Next.js 方案？
2. **实时通信**：专家群聊需要 WebSocket 还是 SSE（Server-Sent Events）即可？
3. **多用户协作**：是否需要支持多人同时编辑？群聊模式下多人协作复杂度大增
4. **部署方案**：Docker Compose 一键部署？对个人用户是否太重？
5. **API 设计**：RESTful 还是直接 WS+SSE？各层流式输出如何统一？
6. **会议结构注册**：新会议协议的安装方式是配置文件声明还是插件目录自动发现？
7. **专家发言格式化**：群聊中的 Markdown 表格/代码块如何优雅渲染？
...
世界书说用 JSON，但整个系统的数据怎么存：

- **项目结构**：一个项目一个目录？SQLite 单文件？项目数量多时如何管理？
- **中间产物**：L1 愿景、L2 大纲、L3 细纲、L4 正文各自是什么格式？JSON/YAML/Markdown？放在哪？
- **版本历史**：世界书的 commit 链、大纲的修改记录，用什么存储？（Git 语义但不需要 Git）
- **配置存储**：用户配置、标签库自定义、专家提示词模板
